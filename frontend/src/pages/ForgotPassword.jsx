...
import BrandLogo from "../components/LogoMark"; // Import BrandLogo
import { useTheme } from "../context/ThemeContext"; // Import useTheme

export default function ForgotPassword() {
  const navigate = useNavigate();
  const { theme } = useTheme(); // Get theme from context
  const appAccentColor = theme?.vars?.["--app-accent"]; // Extract accent color
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-5 py-10">
      <div className="w-full max-w-md">
        ...
        <p className="mt-3 text-stone-600">
          Enter your account email and we&apos;ll send you a short-lived link to
          set a new one. The link works for <strong>30 minutes</strong>.
        </p>
        ...
        {!sent ? (
          ...
          >
            <div className="flex items-start gap-3">
              <MailCheck className="h-5 w-5 text-success mt-0.5" />
              <div>
            ...
            {sent.preview_reset_link && (
              <div className="mt-5 rounded-xl border border-dashed border-primary/40 bg-secondary/60 p-4">
                <p className="text-[10px] uppercase tracking-[0.22em] text-primary">
                  Preview mode · no email yet
                </p>
                <p
                  className="mt-2 text-xs text-stone-600 break-all"
                ...
                <div className="mt-3 flex gap-2">
                  <Button
                    onClick={() => copy(sent.preview_reset_link)}
                    variant="outline"
                  ...
                  <Button
                    onClick={() =>
                      navigate(
                        new URL(sent.preview_reset_link).pathname +
                          new URL(sent.preview_reset_link).search,
                      )
                    }
